...
function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const redirectUrl = localStorage.getItem("inviteRedirect");
      if (redirectUrl) {
      ...
      console.log(error);
      alert(error.response?.data?.message || "Failed to log in. Please verify credentials.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#09090b] relative overflow-hidden flex items-center justify-center px-4">
      ...
      <form
        onSubmit={onSubmit}
        className="w-full max-w-md bg-[#121215]/90 backdrop-blur-md border border-gray-800 p-8 shadow-2xl rounded-2xl z-10 flex flex-col gap-6"
      >
        {/* Logo and title */}
        <div className="flex flex-col items-center gap-2 text-center mb-2">
          ...
          <p className="text-sm text-gray-400 font-mono">Log in to your CodeCollab account</p>
        </div>
        ...
        <div className="text-sm text-center text-gray-400 mt-1">
          ...
          <Link className="text-indigo-400 hover:text-indigo-300 font-semibold transition" to="/register">
            Register
          </Link>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 cursor-pointer shadow-lg hover:shadow-indigo-500/10 text-sm flex items-center justify-center"
        >
          {loading ? (
